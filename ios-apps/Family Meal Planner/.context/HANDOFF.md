...
- **Both queued items CODE DONE on branch `empty-week-and-region`** (do not merge until a device pass). Build number untouched (118 uploaded). **Suite: 210 tests, 0 failures** (200 + 4 `WeekFooterTests` + 6 `StateRegionMapTests`); zero compiler warnings; install + launch verified on iPhone 17 AND iPad Pro 11" sims (sign-in screen only).
...
- **The planning FOOTER replaces it** — below the day rows, above "Build the grocery list": the seasonal strip (unchanged rows, now drawing its own 22pt insets) + "Copy last week". **New pure `Models/WeekFooter.swift`** owns the visibility rule: footer shows iff the displayed week has ≥1 OPEN night (WeekSummary's tested rule) and isn't past. Past weeks keep PASSED rows, no "+", no footer; the open-nights line is untouched. **Deliberate narrowing, revisit if it bites:** the old `hasOpenFutureDay` copy-link gate (which also counted open MEMBER slots) is retired — a week with all seven household slots full is "settled" and shows no Copy last week even if member slots remain; an open night implies an open household slot, so inside a visible footer the link always shows.
