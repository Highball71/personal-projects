...
## 2026-09-06 — recipe-delete guard + grocery merge (Home iMac, autonomous, branch `grocery-merge-and-recipe-delete` — NOT merged to main)

Two bugs from the 2026-09-06 Costco trip on build 119, fixed on branch `grocery-merge-and-recipe-delete` (pushed; David merges after the device pass). Build number untouched. **Suite: 224 tests, 0 failures** (212 + 12 new); zero new compiler warnings; no live Anthropic/proxy calls (nothing here touches that path); Supabase MCP was read-only AND signed into the Placatto-only account, so the Shepherd's Pie rows were not inspected — the SELECT for David is in the session summary.

- **Bug 1 — "scheduled in the meal plan" delete refusal, root cause:** `MealPlanService.isRecipeScheduled` counted meal_plans rows on ANY date, but removal is per-row and past days are read-only in the week view — so one past-week entry blocked deletion forever (force-quit irrelevant; the rows are server-side). The schema has no archived/soft-delete state: removal is a hard DELETE and "past" only means date < today. Fixes:
  - Guard now blocks only on LIVE entries (`date >= today`, same UTC isoDate the rows are written with). Past entries never block; deleting a recipe leaves them as recipe-less history rows (meal_plans.recipe_id is ON DELETE SET NULL → they render as "MEAL NEEDS ATTENTION" on past weeks — cosmetic, revisit if it grates).
  - `removeMeal` was also unsafe: it settled groceries FIRST, then deleted without verification — an RLS-blocked delete (PostgREST reports success on 0 rows) silently "succeeded" in the UI with groceries already stripped. Now: snapshot contributions → delete with `.select()` verification (0 rows = surfaced error) → settle. `GroceryService.removeContributions(forMealPlan:)` deleted (removeMeal was its only caller).
  - The blocked alert gained **"Remove from Meal Plan and Delete"** (new `MealPlanService.removeScheduledMeals(for:)` removes every live entry with per-meal grocery settle, then `deleteRecipe`) — no more bouncing between screens. Failures land in the action error banner, partial-failure wording included.
- **Bug 2 — triple olive oil, root cause:** the grocery merge key was (lowercased name, lowercased unit), so "2 tbsp olive oil", "1/4 cup olive oil", and "1 tbsp extra-virgin olive oil" were three keys. New pure model **`Models/GroceryMerge.swift`** (all rules unit-tested):
  - Merge key = normalized NAME only (trim/lowercase/collapse whitespace + a tiny EXPLICIT alias table — currently just extra-virgin/extra virgin/EVOO → olive oil; extend entry-by-entry, never fuzzy — "green onion" stays separate from "onion").
  - Compatible units convert within a family and sum: tsp/tbsp/fl oz/cup, oz/lb, g/kg, mL/L (mL/L added beyond the spec's three families — unambiguous). **The existing row's unit never changes** (first unit wins), so grocery_contributions stay in the row's unit and the unwind math stays consistent; contributions are recorded converted. The Costco case lands as one row, **"7 tbsp"** (formatAsFraction(7) = "7" — clean, so no "1/4 cup + 3 tbsp" split needed).
  - Incompatible units ("1 piece" + "2 tbsp") stay ONE row: extra amount goes to the new `grocery_items.note` column, displayed "1 piece + 2 tbsp". Its contribution is recorded as 0 (the amount lives in text, the unwind must not subtract it); removing that meal later leaves the note text — informational, accepted.
  - Pre-existing duplicate rows already in the DB are NOT retroactively consolidated (new adds merge into the first unchecked row per name); they clear as they're checked off.
- **⚠️ Migration `015_ingredient_and_grocery_notes.sql` written, NOT applied — David applies before the device pass.** Adds nullable `note` text to `recipe_ingredients` (a home for printed package sizes / quantity ranges instead of the name — the photo-import branch writes it after its rebase onto this) and to `grocery_items` (the incompatible-amounts text). The app tolerates the un-applied migration (nil decode, key omitted when nil), but an incompatible-unit merge before it's applied fails with an honest error. Wired: `RecipeIngredientRow/Insert.note`, `SupabaseGroceryItem/GroceryItemInsert.note`, `IngredientFormData.note` through form save/edit-load, recipe-detail and grocery rows display it.
- **`photo-import-fixes` untouched** — rebase it onto this after merge (its range/package text moves into `recipe_ingredients.note`).
- **New tests (12):** `RecipeDeleteGuardTests` (5 — live entry blocks; allows after removal; past-only entries don't block; RLS-silent delete surfaces error and leaves groceries untouched; remove-scheduled removes only live entries), `GroceryMergeTests` (5 — three olive oils → 7 tbsp; incompatible units one row both amounts; onion vs green onion separate; alias/whitespace normalization; unit families incl. oz≠fl oz), `GroceryMergeServiceTests` (2 — cross-recipe adds land on one row via the DB path; incompatible add notes the existing row). Fake store gained `rlsDeleteBlockedTables` (DELETE succeeds, affects 0 rows — the RLS emulation).
- **Device pass owed (on this branch, after applying 015):** (1) replan three recipes sharing a staple (olive oil) and check the grocery list shows ONE row with a sane total; (2) remove a recipe from the meal plan, then delete it — and try delete-while-scheduled to see the one-step "Remove from Meal Plan and Delete" action work end to end.
- 2026-09-06 later: device pass PASSED, migration 015 APPLIED to production, branch merged to main (36cfa1c). The bullets above describing "NOT applied"/"NOT merged" are historical.

---

## 2026-09-06 — photo-import fixes (Home iMac, autonomous, branch `photo-import-fixes` — NOT merged to main)

- **All three confirmed defects from the 2026-09-06 photo-import test report (build 119) are FIXED on branch `photo-import-fixes`.** David merges after the device pass. Build number untouched — **NEXT BUILD = 120.** **Suite: 224 tests, 0 failures** (212 + 12 new); zero new compiler warnings; all verification offline against saved fixtures (fake transport — no Anthropic/proxy calls were made this session).
- **Fix 1 — truncated responses:** `AnthropicModels.maxTokens` 2048 → 8192 (P05 needed 2,403 output tokens). `RecipeImageExtractor` now checks `stop_reason`: on `max_tokens` it retries ONCE at double the limit (16,384) instead of handing cut-off JSON to the parser; a second truncation throws `ExtractionError.responseTruncated`, which surfaces as "The recipe was too long to read in one pass — try fewer pages or a closer photo" (not the generic parse-failure message). `AnthropicClient` gained an injectable `transport` (production = URLSession; tests = canned responses). **Extended thinking is NOT enabled in the app's requests** (no `thinking` param is sent) — P10's 639 thinking tokens came from the diagnostic harness, nothing to change; the raised limit is pure JSON budget.
- **Fix 2 — quantity ranges:** new `ParsedQuantity` enum (`exact` / `range(min,max)` / `unspecified`) on `ExtractedIngredient` replaces the Double-only `quantityDouble` (deleted). Handles unicode fractions ("1¼–1½") and "to"/-/–/— ranges. A range row's numeric quantity is the UPPER bound (grocery aggregation never under-buys); the printed range shows in the form's quantity text and lives in `recipe_ingredients.note` ("1 1/4 to 1 1/2 lb"). Nothing falls back to 1 anymore.
- **Fix 3 — package sizes + unspecified amounts:** "1 bag (14 ounces)" keeps count 1 + container-word unit (new `IngredientUnit.bag` case; can/package already existed) with the parenthetical size in `note`. Ingredients with no printed amount (sesame oil P12, parsley P13, P05's garnishes) become `.unspecified` → unit `.toTaste` (form hides the qty field; grocery row reads "to taste") — never an invented "1 piece"; if the page printed unparseable text ("to taste", "for garnish"), that text is preserved in `note`.
- **2026-09-06 later (post-grocery-merge rebase):** branch rebased onto main `36cfa1c` (only HANDOFF.md conflicted). The original fix folded range/package text into the ingredient NAME (pre-015 there was nowhere else); after the rebase all of it moved to the `note` column, so **names stay clean and the grocery merge matches a scanned "extra-virgin olive oil" with a hand-entered "olive oil"** (alias table) — `ScannedIngredientMergeTests` proves it end to end. Also fixed the **P15 gap:** servingSize "8 to 10" now parses with the same range parser — servings = UPPER bound (10), printed text folded to the top of the form's notes as "Serves 8 to 10" (no dedicated servings-note field exists); "4-6" now yields 6, not 4.
- **Tests: 240, 0 failures** (main's 224 + 12 branch + 4 rebase-session): `PhotoTruncationRetryTests` (2), `QuantityRangeConversionTests` (5 — incl. P15 servings range), `PackageSizeConversionTests` (6 — incl. printed-unparseable-text preservation), `BetaFormConversionReplayTests` (1 — no flagged ingredient becomes 1 piece AND no amount-derived text leaks into a name), `ScannedIngredientMergeTests` (1). Updated: `ExtractedIngredientTests`, `IngredientUnitTests` (+`.bag`), `ExtractedRecipeTests` (servings ranges → upper bound).
- **Known issue (out of scope, do NOT hardcode a correction):** see "Photo import — extraction accuracy" below.
- **Device pass owed on this branch (then NEXT BUILD = 120):** scan P04 → beef row shows the clean name, range in note, grocery quantity 1 1/2 lb; scan P11 → bag/can rows with sizes in note; scan P12/P13 → to-taste rows with blank qty; then plan a recipe using olive oil and scan one with extra-virgin olive oil — the grocery list must show ONE olive-oil row.

## Known issues / photo import — extraction accuracy

- **P05 tomato quantity misread ("3/4 vs 1/4 cup" per the 2026-09-06 test report):** extraction-quality issue in the vision model's reading, NOT a conversion bug — no code correction is appropriate and nothing is hardcoded. Any prompt/model tuning attempt should be evaluated against the same saved originals (`Documents/Codex/2026-09-05/…/outputs/Recipe Test Photos/Originals`) so regressions are measurable.
- 2026-09-06 evening: a prompt-level mitigation shipped on branch `deleted-recipe-label-and-fraction-prompt` (see below) — still no recipe-specific hardcoding; the Codex re-review scores it against the ten originals before it counts as fixed.

---

## 2026-09-06 evening — deleted-recipe history label + extraction-accuracy prompt (Home iMac, autonomous, branch `deleted-recipe-label-and-fraction-prompt` — NOT merged to main)

Two small items on build 120's main; build number untouched. **Suite: 248 tests, 0 failures** (240 + 8 new); zero new compiler warnings; no live Anthropic calls (the prompt change gets scored by a separate Codex accuracy re-review).

- **1. Deleted-recipe rows are history, not to-dos.** Surprise found while implementing: `DayPlan.build` DROPPED nil-recipe rows, so after a refetch a deleted recipe's meals vanished from the week entirely (the "MEAL NEEDS ATTENTION" David saw was the stale-cache window where the in-memory row still held the old recipe id). Now: `DayPlan` keeps orphaned rows ("nothing planned is ever invisible"), and the new pure `MealLineState` (in DayPlan.swift) decides the face — **past + recipe_id NULL → "(recipe deleted)"** in italic tertiary, NOT tappable, no swipe (past days never had swipe); **today/future + recipe_id NULL → the existing "MEAL NEEDS ATTENTION"** treatment (tap → Replace, swipe works). Unresolved-but-non-null recipe ids keep attention treatment on any date (stale-cache window). Side effects accepted and deliberate: an orphan row now counts as occupying its slot (WeekSummary open-nights, "Nothing for everyone" placeholder suppressed, Clear-Whole-Day counts) — resolving the attention row is the honest path; copyPreviousWeek already filters nil-recipe rows at its own layer and is unaffected.
- **2. Extraction-accuracy prompt tightening (P05 "3/4 vs 1/4 cup", P02 dropped "unsweetened"/"in pineapple juice") — nothing recipe-specific:** `schemaInstructions` now demands character-for-character transcription of quantities (never round/simplify/reinterpret a fraction, re-read small or blurry ones), VERBATIM ingredient qualifiers (dropped/paraphrased qualifiers forbidden), and a new optional `printedAmount` field (the quantity text exactly as printed). `ExtractedIngredient.printedAmount` decodes it (optional — all twelve saved fixtures still decode, pinned by test) and wins over `amount` for parsing and note text when present. **Image resize raised 1200 → 1568 px** longest side: at 1200 a two-column page is ~133 DPI, a 5-6 pt fraction glyph ~10 px — plausibly under-resolved for 1/4-vs-3/4; 1568 is the API's max-kept long edge (the requested 1600 would be server-downscaled to 1568, re-sampling twice for nothing). Cost: image input tokens scale with pixel area, ~1.7x per photo (~1.4k → ~2.5k tokens). maxTokens untouched.
- **New tests (8):** `DeletedRecipeLineTests` (3 — past nil-recipe row fetched through the fake backend renders as quiet history; live nil-recipe row keeps attention; unresolved non-null id always attention), `ExtractionPromptTests` (5 — prompt contains the fraction and qualifier rules; all twelve fixtures still decode with printedAmount nil; printedAmount preferred over amount; printedAmount decodes from JSON). Updated: `SlotSemanticsTests.testDayPlanKeepsRowsWithoutRecipe` (was ...Drops...).
- **Device pass owed on this branch:** delete a recipe that has a past-week entry → the past week shows "(recipe deleted)" muted and inert; a live entry for a deleted recipe still shows "MEAL NEEDS ATTENTION" and Replace works. **NEXT BUILD = 121 only after the Codex accuracy re-review scores the prompt change against the ten saved originals.**

### 2026-09-06 night — Codex round-2 fixes (same branch, Home iMac, autonomous)

The Codex round-2 review ("Photo review round 2/Review.md" beside the round-1 evidence; 8/10 imports, P12/P13 newly failed on JSON null) drove four fixes. **Suite: 257 tests, 0 failures** (248 + 9 new); zero new compiler warnings; all offline (round-2 responses replayed from `PhotoImportFixtures/round2/` — 10 raw responses, 8 extracted recipes, the P04 form snapshot).

- **1. Null amounts no longer abort recipes:** `ExtractedIngredient.amount`/`unit` are optional; null, missing, and "" all mean unspecified → to-taste row. The prompt now asks for "" (never null) for absent quantities, but the decoder tolerates null regardless. Both failed round-2 responses (P12 sesame oil, P13 parsley) decode and convert, pinned by tests that replay the raw proxy envelopes (P13's leading thinking block included).
- **2. Containers survive printedAmount:** round-2 P11 put the container phrase in printedAmount ("1 bag (14 ounces)") with the CONTENTS in amount/unit ("14 ounces"), and printedAmount-wins parsing turned fully specified packages into "to taste". New `parseContainerExpression` runs on the parsing string (count + bag/can/package word + parenthetical size; "1 1/2 pounds" can never match) via the consolidated `resolvedQuantity`. Also: a nonempty printed string that doesn't parse falls back to the structured `amount` before unspecifying — an unrecognized printed string never silently becomes to-taste when a usable number exists.
- **3. Ingredient notes reach the grocery list:** `addMealWithGroceries` and the form's add-to-grocery now carry `recipe_ingredients.note` into `grocery_items.note`; on merge, differing notes join with "; " (`GroceryMerge.joinedNotes` — never overwritten, never dropped, duplicates not stacked). Planning the round-2 P04 snapshot puts "1¼ to 1½ lb" on the meat's grocery row.
- **4. Section labels moved from name to note:** form conversion no longer folds section/preparation into the ingredient name (the "[Sauce] onion" fold split grocery identity). Section becomes a "Marinade: …" note prefix, preparation joins the note body; the name stays the bare pantry item, so the same ingredient across two sections of one recipe AND across recipes merges to one grocery row. The replay test now also asserts no name carries a section label and every section reaches the note. (P05's duplicated "minced" in name+preparation is the model's own text — name content is never stripped.)
