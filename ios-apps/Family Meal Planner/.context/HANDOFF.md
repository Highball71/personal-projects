...
- **Fix 2 — quantity ranges:** new `ParsedQuantity` enum (`exact` / `range(min,max)` / `unspecified`) on `ExtractedIngredient` replaces the Double-only `quantityDouble` (deleted). Handles unicode fractions ("1¼–1½") and "to"/-/–/— ranges. A range row's numeric quantity is the UPPER bound (grocery aggregation never under-buys) and the printed range stays visible twice: the form's quantity text shows it, and it's folded into the ingredient name ("beef sirloin… (1 1/4 to 1 1/2 lb)") — the DB row only persists (quantity, unit), so the name is what carries the range to the saved recipe and grocery list. Nothing falls back to 1 anymore.
- **Fix 3 — package sizes + unspecified amounts:** "1 bag (14 ounces)" keeps count 1 + container-word unit (new `IngredientUnit.bag` case; can/package already existed) with the parenthetical size folded into the name. Ingredients with no printed amount (sesame oil P12, parsley P13, P05's garnishes) become `.unspecified` → unit `.toTaste` (form hides the qty field; grocery row reads "to taste") — never an invented "1 piece".
- **New tests (12):** `PhotoTruncationRetryTests` (2 — truncate-then-retry parses P05; double-truncation throws responseTruncated, exactly one retry), `QuantityRangeConversionTests` (4 — range spellings; mixed-number guard; P04 + P10 meat round-trips as range(1.25, 1.5) unit lb), `PackageSizeConversionTests` (5 — P11 bag/can, P13 package, P12/P13/P05 unspecified), `BetaFormConversionReplayTests` (1 — replays all ten build-119 form snapshots; no range/package/missing-amount ingredient ends up 1/piece). Fixtures = the saved test-task evidence, copied to `Family Meal PlannerTests/PhotoImportFixtures/` (8 baselines, P05+P10 4096-token diagnostics, Beta-form-conversion.json, P02-reference.json). Updated: `ExtractedIngredientTests` (parsedQuantity; unparseable/empty now `.unspecified`, not 1.0), `IngredientUnitTests` counts (+`.bag`).
- **Known issue (out of scope, do NOT hardcode a correction):** see "Photo import — extraction accuracy" below.
- **Device pass owed before merge/120:** scan the P04 and P11 photos on a real device and check the recipe form + grocery list — P04 beef shows the printed range (form qty text + name suffix, grocery quantity 1 1/2 lb with range in the name); P11 bag/can rows show "1 bag"/"1 can" with sizes in the name; a garnish row reads "to taste" with a blank qty field.
