    ...
    var body: some View {
        NavigationStack {
            VStack(spacing: 30) {
                ...
                // Cadence & Metronome settings
                VStack(spacing: 16) {
                    VStack(alignment: .leading, spacing: 8) {
                        Text("Target Cadence: \(Int(targetCadence)) SPM")
                            .font(.headline)
                        Slider(value: $targetCadence, in: 140...210, step: 1)
                            .tint(.orange)
                    }
                    ...
                    VStack(alignment: .leading, spacing: 8) {
                        Text("Metronome")
                            .font(.headline)
                        Picker("Metronome Mode", selection: $metronomeMode) {
                            ForEach(MetronomeMode.allCases) { mode in
                                Text(mode.rawValue).tag(mode)
                            }
                        }
                        .pickerStyle(.segmented)
                        Text(metronomeMode.description)
                            .font(.caption)
